...
# --- 1️⃣ صفحة المتجر الرئيسي المتطور (الواجهة الذكية الجديدة مدمجة بالكامل) ---
HTML_INDEX = """
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
...
<body>
    ...
    <script>
        ...
        // تحويل بيانات البايثون القادمة من السيرفر لـ JSON يفهمه الجافاسكريبت
...
<body>
    ...
    <script>
        ...
        function submitMarketerOrder(e) {
            ...
            window.location.href = `https://api.whatsapp.com/send?phone=${MY_WHATSAPP_NUMBER}&text=${encodeURIComponent(msg)}`;
        }
    </script>
</body>
</html>
...
<head>
    ...
    <style>
        ...
        .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 10px; background: #171717; border: 1px solid #525252; border-radius: 6px; box-sizing: border-box; color: white; text-align: right; font-size: 14px; }
        ...
        table { width: 100%; border-collapse: collapse; text-align: center; font-size: 13px; min-width: 600px; }
    ...
    <div class="container">
        <div class="card border-yellow">
            ...
            <form action="/add_product" method="POST" id="product-form">
                ...
                <div class="form-group">
                    <label>قسم المنتج (لتصنيف وتسهيل البحث للزبون):</label>
                    <select name="category">
                        <option value="ملابس">👕 ملابس</option>
                        <option value="أحذية">👟 أحذية</option>
                        <option value="ساعات وعطور">⌚ ساعات وعطور</option>
                        <option value="إلكترونيات">📱 إلكترونيات</option>
                        <option value="عام" selected>📦 عام / متنوع</option>
                    </select>
                </div>
        ...
        <div class="card" style="border-top: 5px solid #2563eb;">
            <h2 style="color: #2563eb;">📥 الطلبات المباشرة المسجلة أونلاين (تحتاج شحن)</h2>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>الكمية</th>
                            <th>الإجمالي</th>
                            <th>العنوان تفصيلياً</th>
                        </tr>
                    </thead>
                    <tbody>
                        {% for o in orders %}
                        <tr>
                            <td style="font-weight: bold; color: white;">{{ o.customer_name }}</td>
                            <td>{{ o.customer_phone }}</td>
                            <td style="color:#2563eb;">{{ o.product_name }}</td>
                            <td>{{ o.quantity }} قطع</td>
                            <td style="color: #22c55e; font-weight:bold;">{{ o.total_price }} ج.م</td>
                            <td>{{ o.customer_address }}</td>
                        </tr>
                        {% else %}
                        <tr><td colspan="6" style="color: #a3a3a3; padding: 15px;">لا توجد طلبات مباشرة مسجلة حالياً.</td></tr>
                        {% endfor %}
                    </tbody>
                </table>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="color: #a3a3a3;">{{ p.category }}</td>
                        ...
                        {% else %}
                        <tr><td colspan="6" style="color: #a3a3a3; padding: 20px;">المخزن فارغ تماماً حالياً.</td></tr>
                        {% endfor %}
                    </tbody>
                </table>
            </div>
...
def fetch_all_products():
    ...
    cur.execute("SELECT id, name, category, description, purchase_price, selling_price, commission, stock_quantity, image_url FROM products ORDER BY id DESC;")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return rows
...
def fetch_all_orders():
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("SELECT product_name, quantity, total_price, customer_name, customer_phone, customer_address FROM orders ORDER BY id DESC;")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return rows
...
def index():
    ...
    # تحويل المنتجات لـ JSON مسبقاً عشان نبعتها للجافا سكريبت بشكل سليم تماماً
    json_products = json.dumps(products)
    return render_template_string(HTML_INDEX, products=products, json_products=json_products, whatsapp=MY_WHATSAPP_NUMBER)
...
def cashier():
    ...
    return render_template_string(HTML_CASHIER, products=products, orders=orders)
...
def add_product():
    ...
    if row:
        ...
        cur.execute("""
            UPDATE products 
            SET stock_quantity = %s, image_url = %s, description = %s, category = %s, purchase_price = %s, selling_price = %s, commission = %s 
            WHERE name = %s;
        """, (new_qty, final_img, final_desc, category, purchase_price, selling_price, commission, name))
    else:
        cur.execute("""
            INSERT INTO products (name, category, description, purchase_price, selling_price, commission, stock_quantity, image_url) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s);
        """, (name, category, description, purchase_price, selling_price, commission, quantity, image_url))
...
# API جديد خاص باستقبال وحفظ الطلبات المباشرة للعملاء المسجلين
...
def create_order_api():
    ...
    cur.execute("""
        INSERT INTO orders (product_id, product_name, quantity, total_price, customer_name, customer_phone, customer_address)
        VALUES (%s, %s, %s, %s, %s, %s, %s);
    """, (data['product_id'], data['product_name'], data['quantity'], data['total'], data['customer_name'], data['customer_phone'], data['customer_address']))
    
    # تحديث وتقليل الكمية في المخزن فورياً عند الشراء المباشر
    cur.execute("UPDATE products SET stock_quantity = stock_quantity - %s WHERE id = %s;", (data['quantity'], data['product_id']))
