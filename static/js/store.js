...
async function loadStoreProducts() {
    try {
        ...
        if (container) {
            container.innerHTML = `<p style="grid-column: 1/-1; text-align:center; color: red; font-weight:bold;">❌ فشل الاتصال بالمخزن السحابي.</p>`;
        }
    }
}
...
function renderProducts(products) {
    ...
    products.forEach(p => {
        ...
        let stockText = p.stock_qty > 0 ? `المخزون: ${p.stock_qty}` : `نفذت الكمية`;
        container.innerHTML += `
